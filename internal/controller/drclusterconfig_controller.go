...
func (r *DRClusterConfigReconciler) validateS3Profiles(ctx context.Context, log logr.Logger, drCConfig *ramen.DRClusterConfig) error {
	...
	for profileIdx := range ramenConfig.S3StoreProfiles {
		...
		if reason, err := s3ProfileValidate(ctx, r.APIReader, r.ObjectStoreGetter, profile.S3ProfileName, types.NamespacedName{
			Name: drCConfig.Name, Namespace: drCConfig.Namespace}.String(), r.Log); err != nil {
