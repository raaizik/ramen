...
class Tool:
    ...
    def install_dir(self):
        """
        Return the venv bin directory where tools are installed.
...
class Kustomize(Tool):
    ...
    def version(self):
        ...
        try:
            ...
            # Check if it's a FileNotFoundError (tool not installed)
            # commands.Error.with_exception() sets __cause__ to None, so we check the error message
            if "No such file or directory" in str(e.error):
                return None
            # Other errors should propagate - don't hide bugs
            raise
    ...
    def install(self):
        ...
        url = (
            ...
            f"releases/download/kustomize%2Fv{self.required_version}/"
            f"kustomize_v{self.required_version}_{OS_NAME}_{ARCH}.tar.gz"
        )
        ...
        logging.info(f"Installed {self.name} {self.required_version}")
...
def setup(install_dir=None):
    ...
    logging.info("Checking and installing required tools...")

    for tool in tools:
        logging.info(f"Checking {tool.name}...")
        current = tool.version()

        if current is None:
            logging.info(
                f"  {tool.name} not found, installing {tool.required_version}..."
            )
            try:
                tool.install()
            ...
            logging.info(
                f"  {tool.name} {current} < {tool.required_version}, upgrading..."
            )
            try:
                tool.install()
            except Exception as e:
                logging.error(f"  Failed to upgrade {tool.name}: {e}")
                raise
        else:
            logging.info(f"  {tool.name} {current} is up to date")

    logging.info("All tools installed successfully")
